...
5. Nachdem die Schleife endet, sollen alle Eingaben des Nutzers der Reihe nach auf der Konsole ausgegeben werden (jede Zahl ein `print`).
...
3. Wandle einen Teil deines bisheriges Skripts in zwei Funktionen um: `getPartner` und `printPartner`.\
...
Es ist möglich, dass sich eine Person selbst zieht.
Löse, oder umgehe das Problem.\
Du darfst dabei so viele zusätzliche Funktionen schreiben wie du willst.\
Tipp: Mir fallen dazu 4 stark unterschiedliche Lösungen ein. Eine davon ist super einfach zu implementieren, aber schwierig zu finden.\
